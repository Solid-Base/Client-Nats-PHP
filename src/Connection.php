...
class Connection
{
     ...
     * @return resource
     */
    public function getStreamSocket(): mixed
    {
     ...
     * @return resource
     */
    private function getStream(string $address, float $timeout, mixed $context): mixed
    {
        $errno  = null;
        $errstr = null;
        ...
        if ($fp === false) {
            throw NatsException::forStreamSocketClientError($errstr, $errno);
        }
     ...
     *
     * @throws             NatsException If subscription not found.
     * @return             void
     * @codeCoverageIgnore
     */
    private function handleMSG(string $line): void
    {
        $parts   = explode(' ', $line);
        $subject = null;
        $length  = trim($parts[3]);
        ...
        if (isset($this->subscriptions[$sid]) === false) {
            throw NatsException::forSubscriptionNotFound($sid);
        }
        ...
        if (is_callable($func) === true) {
            ...
            throw NatsException::forSubscriptionCallbackInvalid($sid);
        }
    }
    ...
    public function connect(float $timeout = null): void
    {
        ...
        if ($this->isErrorResponse($infoResponse) === true) {
            throw NatsException::forFailedConnection($infoResponse);
        } else {
            ...
            if ($this->serverInfo->isTLSRequired()) {
                set_error_handler(
                    function ($errno, $errstr, $errfile, $errline) {
                        restore_error_handler();
                        throw NatsException::forFailedConnection($errstr);
                    }
                );
                ...
                if (!stream_socket_enable_crypto(
                    ...
                    throw NatsException::forFailedConnection('Error negotiating crypto');
                }
        ...
        if ($this->isErrorResponse($pingResponse) === true) {
            throw NatsException::forFailedPing($pingResponse);
        }
    }
     ...
     */
    public function wait(int $quantity = 0): self
    {
        $count = 0;
